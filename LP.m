...
[s.window, s.windowRect] = PsychImaging('OpenWindow', screenNumber, s.grey);
...
[s.xc, s.yc] = RectCenter(s.windowRect);
s.winw = windowRect(3);
s.winh = windowRect(4);
...
function s = makeBoxOfCards(s)
% make box of cards
box_front = [0 0 s.winw/5 s.winw/5*0.7];
box_front_pos = CenterRectOnPointd(box_front, s.winw/6, s.winh/3);
box_front_color = 0.9*s.white;
% the top and the right side of the box are polygons (point lists)
hoffset = box_front(3)/5;
voffset = box_front(4)/3;
...
% make a slit on top
box_slit = [box_front_pos(1)+box_front(3)/4,box_front_pos(2)+voffset*2/5;
  box_front_pos(1)+box_front(3)*3/4,box_front_pos(2)+voffset*2/5;
  box_front_pos(1)+box_front(3)*3/4+hoffset/2.box_front_pos(2)+voffset*3/5???????????????????????????????????????????????????????????
...
s.box_front_color = box_front_color; s.box_top_color = box_front_color;
